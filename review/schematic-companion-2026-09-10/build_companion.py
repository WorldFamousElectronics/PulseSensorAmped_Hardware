...
def make(path,bleed):
 ...
 for y,n,lab,col in [(168,'1','GND','cyan'),(186,'2','+V','red'),(204,'3','OUT','violet')]:t(74,y-3,n,13.5);l([(88,y),(110,y)],col);t(115,y+4,lab,14,True,col)
 t(226,144,'+V',14,True,'red',a='center');l([(226,148),(226,154)],'red');cap(226,174,'C5','2.2uF',True);l([(226,194),(226,203)],'cyan');g(226,211)
 box(300,102,276,132);head(314,122,'2  POWER + LED','amber')
 ...
 t(111,279,'PROT',14,True,'amber',a='center');l([(111,281),(111,291)],'amber');t(117,289,'1',13.5)
 l([(111,357),(111,373)],'cyan');t(117,370,'4',13.5);g(111,380)
 for y,n in [(303,'2'),(325,'3'),(347,'5')]:l([(61,y),(77,y)],'soft');l([(57,y-4),(65,y+4)],'soft');l([(57,y+4),(65,y-4)],'soft');t(78,y-3,n,13.5)
 t(38,379,'NC',13,col='soft');l([(149,319),(238,319)],'cyan');t(151,315,'6',13.5);t(179,307,'IOUT',14,True,'violet');dot(216,319,'cyan');l([(216,319),(216,334)],'cyan');res(216,354,'R2','12k','cyan',True);g(216,382)
 ...
 t(38,467,'IOUT',14,True,'violet');l([(74,463),(79,463)],'violet');cap(99,463,'C2','4.7uF');l([(119,463),(166,463)],'violet');cap(186,463,'C3','4.7uF');l([(206,463),(292,463)],'violet');t(294,467,'B',14,True,'violet')
 dot(142,463);l([(142,463),(142,487)],'violet');cap(142,507,'C1','4.7uF',True);l([(142,527),(142,533)],'cyan');g(142,541)
 dot(238,463);l([(238,463),(238,487)],'violet');cap(238,507,'C4','2.2uF',True);t(238,547,'VREF',14,True,'violet',a='center');l([(238,527),(238,533)],'violet')
 ...
 l([(432,485),(432,451),(438,451)],'violet');res(462,451,'R6','','violet');t(530,440,'3.3M',14,col='soft',a='center');l([(486,451),(553,451),(553,498)],'violet')
 l([(454,475),(454,525),(496,500),(454,475)],'violet');t(458,490,'-',15,True);t(458,516,'+',15,True);t(474,505,'U2',14,True,a='center');t(445,482,'4',13.5);t(445,514,'3',13.5)
 l([(496,500),(560,500)],'violet');l([(553,498),(553,500)],'violet');dot(553,500);t(503,495,'1',13.5);t(552,522,'OUT',14,True,'violet',a='center')
 l([(438,516),(454,516)],'violet');t(433,540,'VREF',14,True,'violet',a='right');l([(433,536),(438,536),(438,516)],'violet')
 l([(476,488),(476,472),(500,472)],'amber');t(479,488,'5',13.5);t(504,476,'PROT',14,True,'amber')
 l([(476,512),(476,537)],'cyan');t(482,531,'2',13.5);g(476,545)
 ...
 box(24,109,552,73,'ink');t(40,130,'TL;DR',15,True,'#ffffff');t(40,153,'J1 brings in power and carries OUT back.',20,True,'#ffffff');t(40,176,'U1 + R2 feed the filter. U2 amplifies the result.',17,False,'#ffffff')
 items=[('1','CONNECT + DECOUPLE','J1 is the cable connector. C5 helps steady','the incoming supply. Pins: 1 GND, 2 +V, 3 OUT.'),('2','POWER + LED','D1 is the LED; R1 limits its current. D2 feeds','the protected supply, PROT, to U1 and U2.'),('3','SENSOR + CURRENT LOAD','U1 is the sensor chip. R2 turns its output','current into a voltage at the IOUT connection.'),('4','MAKE A MIDPOINT','R4 and R3 divide +V in half to make VREF.','This reference gives the signal a midpoint.'),('5','FILTER','C2 and C3 are in the signal path. C1 and C4','connect it to GND and VREF, shaping the response.'),('6','AMPLIFY','U2 (MCP6001) amplifies. R6 feeds OUT back','to its - input. R5 / R6 set gain; + uses VREF.')]
 for i,(n,h,a,b) in enumerate(items):
  y=199+i*50;t(27,y,n,19,True,'violet');t(50,y,h,14,True);t(50,y+17,a,15);t(50,y+33,b,15)
 box(24,496,552,63);t(38,515,'THE SYMBOL KEY',14,True)
 t(38,535,'R  resistor',15,True);t(170,535,'C  capacitor',15,True);t(320,535,'D  diode / LED',15,True);t(485,535,'U  chip',15,True)
 t(38,554,'Dot = wires join. Small pin numbers match the chip or connector.',14)
