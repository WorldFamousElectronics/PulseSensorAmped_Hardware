...
R=Path(__file__).resolve().parent;src=R/'mono-08';out=R/'color-09';out.mkdir(exist_ok=True)
...
arrow=unary_union([LineString([a,b]).buffer(.075,quad_segs=8) for a,b in json.loads((R.parent/'sendoff-candidate-2026-09-09/guide-plan.json').read_text())['led_current_arrow']]);guide=guide.difference(arrow.buffer(.001))
changes=json.loads((src/'artwork-change.json').read_text());marks=unary_union([load(m['polygons']) for m in changes['polarity_marks']])
layers += [('signal-guide','#c4a1ff',guide.intersection(allowed)),('labels','#ffffff',ink.difference(guide.buffer(.0005)).difference(marks).difference(arrow.buffer(.001))),('LED current arrow','#ffbe55',arrow)]
...
(out/'net-path-source.json').write_text(json.dumps({'derived_from':'mono-08 native track segments and filled F.Cu GND zone','solid':'F.Cu projection','dashed':'B.Cu projection at same board coordinates','ground':'F.Cu plane; does not prescribe a unique current path','segments':evidence},indent=2))
