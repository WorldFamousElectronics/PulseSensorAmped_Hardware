...
R=Path(__file__).resolve().parent;OLD=R.parent/'sendoff-candidate-2026-09-09';src=OLD/'candidate-05';out=R/'mono-08';out.mkdir(exist_ok=True)
...
for symbol,pad,target in [('-', [99,102.25],[98.65,101.25]),('+',[100,102.25],[100,101.05])]:
 shape=box(-.275,-.085,.275,.085) if symbol=='-' else deserial(cat[symbol]['polygons']); options=[]
 for ix in range(65):
  x=97.6+ix*.05
  for iy in range(27):
