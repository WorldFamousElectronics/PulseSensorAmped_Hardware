...
colors={'FILTER': '#51cf66', 'AMP + GAIN': '#64c8ff', 'BIAS': '#ffaa44', 'LED': '#44ff44', 'SENSE': '#44ff44', 'POWER': '#ff6b6b'}
...
for item in j:
 if item['name'] in ['labels','+','-']:layers.append((item['name'],'#000000',unary_union([p for p in polys(load(item['polygons'])) if p.area>=.008])))
for e in root:
 if e.get('fill') in ['#bcb5a1','#535559','#f2f0df']:layers.append(('unchanged component',{'#bcb5a1':'#c0c0c0','#535559':'#555555','#f2f0df':'#ffffff'}[e.get('fill')],frompath(e)))
def path(g):
...
(R/'led-green-power-red-05-back.svg').write_text(svg)
...
def render(ls,size):
 ...
 return im
render(layers,2400).save(R/'led-green-power-red-05-back.png')
...
d.text((65,40),'PulseSensor · WebSerial bright palette study',font=font(52),fill='#152b34');d.text((65,110),'Sheet 28 · Color identifies a functional area · All parts and optical geometry stay in place',font=font(29),fill='#53666d')
for i,(title,p) in enumerate([('PREVIOUS / WEBSERIAL-04',R.parent/'webserial-palette-2026-09-10/webserial-04-back.png'),('ITERATION / LED-GREEN / POWER-RED',R/'led-green-power-red-05-back.png')]):
