...
d.text((65,42),'PulseSensor / WebSerial colors / board + purchase card',font=font(62),fill='#000000')
d.text((65,126),'Sheet 27 · Color-only iteration · Coral + lime + orange + sky blue + status green',font=font(35),fill='#333333')
assets=[('BOARD BACK / WEBSERIAL-04',r/'webserial-04-back.png'),('CARD / FULL SCHEMATIC',r/'output/card-1.png'),('CARD / PARTS SCHEDULE',r/'output/card-2.png')]
...
im.save(r/'webserial-overview.png');im.save(s/'sheet-27.png')
m=json.loads((s/'sources.json').read_text());title='WebSerial bright board and purchase card';m['designs']=[x for x in m['designs'] if x.get('title')!=title]
a={'number':64,'title':title,'note':'Colors only; board SVG geometry and non-color PDF operators verified identical to prior versions.','sources':{}}
for role,p in [('board',r/'webserial-04-back.svg'),('card',r/'output/pdf/webserial-full-color-full-schematic-retail-purchase-card.pdf'),('palette',r/'palette.json'),('overview',r/'webserial-overview.png')]:a['sources'][role]={'path':str(p),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()}
m['designs'].append(a);(s/'sources.json').write_text(json.dumps(m,indent=2)+'\n')
snippet='<article><h2>Sheet 27 — PulseSensor WebSerial palette</h2><a href="sheet-27.png"><img src="sheet-27.png"></a></article>';p=s/'index.html';p.write_text(p.read_text().replace(snippet,'').replace('</main>',snippet+'</main>'))
